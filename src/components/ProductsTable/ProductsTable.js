...
const ProductsTable = ({ products, sort, handleSort, loading }) => {
  ...
  return (
    <div className='overflow-auto w-11/12 mx-auto border border-base-300'>
          ...
          <tr>
            <th className='grid normal-case'>Imagen</th>
            {
              tableHead.map(({ value, slug }) => (
                value 
                  ? (
                    <th key={value} onClick={() => handleSort(value)} ><button> {slug} { sort.col === value ? sort.order === 'asc' ? '↑' : '↓' : null }</button> </th>
                  )
                  : <th key={slug} className='normal-case'>{slug}</th>
              ))
            }
          </tr>
        </thead>
